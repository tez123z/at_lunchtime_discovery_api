    ...
    class API
      ...
      class << self
        ...
        def handle_required_keys(service, args)
          if required_keys = Google::Maps.required_keys[service]
            missing_keys = required_keys - args.symbolize_keys.filter{|k,v| !v.blank? }.keys
            raise InvalidRequestException, "Missing required parameters `#{missing_keys.join(',')}`" if missing_keys.length > 0
          end
        end
